...
<div class="<?php echo esc_attr( (get_option('sirve_archive_page_style', 'style-1') == 'style-4') ? 'sirve__container--xl' :  'sirve__container'); ?>" <?php if(isset($attributes)){ echo esc_attr( "data-sirveoptions=" ). json_encode(  $attributes );}?>>
    <?php $terms_sirve_category = event_and_tags_sirve_menu_filter( $event, $tags,  $exclude_menu); ?>
                        ...
                        <?php
                            ...
                            if ( !is_wp_error( $terms_sirve_category ) ):
                                ...
                                <li class="sirve__nav-item">
                                    <button class="sirve__nav-link scarch-item <?php if($manuActive == true){ echo esc_attr( 'active' ); $manuActive = false;}  ?>" data-category="all" data-target="#all" data-sirveevent='<?php echo esc_attr( $event )?>' data-sirvetags='<?php echo esc_attr( json_encode(  $tags ) )?>' type="button"> 
                                ...
                                foreach ( $terms_sirve_category as $single_sirve_category ) :   ?>  
                                    <li class="sirve__nav-item">
                                        <button class="sirve__nav-link <?php if($manuActive == true){ echo esc_attr( 'active' ); $manuActive = false;} ?>" data-target="#<?php echo esc_attr( $single_sirve_category['slug'] ) ?>" data-sirveevent='<?php echo esc_attr( $event )?>' data-sirvetags='<?php echo esc_attr( json_encode(  $tags ) )?>' data-category="<?php echo esc_attr( $single_sirve_category['slug'] ) ?>" type="button">
                ...
                <div class="sirve__tab-pane <?php if($postActive == true){ echo esc_attr( 'active' ); $postActive = false;}  ?>" id="all">
                    ...
                    <?php 
                        ...
                        if(!($query->post_count == 0)):
                            sirve_pagination($query, $currentPage, 'all', $event, $tags );
                        else:
